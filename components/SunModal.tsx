...
const SunModal: React.FC<SunModalProps> = ({ data, onClose }) => {
  ...
  // State for the slider (Start at 0 for animation)
  const [sliderValue, setSliderValue] = useState(0);
  const animationRef = useRef<number>(0);

  // Animate from 00:00 to current time on mount
  useEffect(() => {
    ...
    const animate = (timestamp: number) => {
      ...
      // Custom Easing: Cubic Ease Out for smooth deceleration
      const ease = 1 - Math.pow(1 - percentage, 3);
      
      // Target is strictly current time
      ...
      if (progress < duration) {
        ...
        // Ensure we land exactly on current time
  ...
  // Define key time points and their 3-stop gradient colors (Top, Middle, Bottom)
  const timeStops = useMemo(() => {
    const stops = [
      { min: 0, colors: ['#0f172a', '#1e293b', '#0f172a'] },                // Night (Midnight)
      { min: Math.max(0, sunriseMins - 90), colors: ['#1e1b4b', '#312e81', '#0f172a'] }, // Pre-Dawn
      { min: sunriseMins - 30, colors: ['#312e81', '#7c3aed', '#f97316'] }, // Dawn
      { min: sunriseMins, colors: ['#60a5fa', '#fb923c', '#fef08a'] },      // Sunrise
      { min: sunriseMins + 60, colors: ['#38bdf8', '#7dd3fc', '#bae6fd'] }, // Morning
      { min: (sunriseMins + sunsetMins) / 2, colors: ['#0ea5e9', '#38bdf8', '#e0f2fe'] }, // Noon
      { min: sunsetMins - 60, colors: ['#38bdf8', '#7dd3fc', '#bae6fd'] }, // Late Afternoon
      { min: sunsetMins, colors: ['#2563eb', '#f59e0b', '#ef4444'] },      // Sunset
      { min: sunsetMins + 45, colors: ['#4c1d95', '#312e81', '#0f172a'] }, // Dusk
      { min: Math.min(1440, sunsetMins + 90), colors: ['#0f172a', '#1e293b', '#0f172a'] }, // Night
      { min: 1440, colors: ['#0f172a', '#1e293b', '#0f172a'] }              // Midnight Next Day
    ];
    // Sort to be safe
  ...
  const bgStyle = useMemo(() => {
    // Find which interval current sliderValue falls into
  ...
  // We allow t to go slightly outside 0-1 range for fade-in/out effect near horizon
  let t = -0.1; // Default off-screen
  let isDay = false;
  let opacity = 0;
  ...
  // Calculate coordinates based on t (quadratic bezier)
  const mt = 1 - t;
  ...
  return createPortal(
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 animate-in fade-in duration-300">
      ...
      <div 
        className="relative w-full max-w-sm text-white rounded-3xl overflow-hidden shadow-2xl border border-white/10 flex flex-col transition-all duration-75 ease-linear"
        style={bgStyle} // Applied calculated gradient here
      >
        ...
        <div className="p-6 flex flex-col items-center">
          ...
          <div className="text-center mb-6">
            ...
            <div className="text-sm font-medium opacity-80">
              {sliderValue < sunriseMins ? '日出前' : 
               sliderValue > sunsetMins ? '日落后' : 
               '白昼'}
            </div>
          </div>
          ...
          <div className="w-full h-40 relative flex items-center justify-center mb-6">
             <svg viewBox={`0 0 ${width} ${height}`} className="w-full h-full overflow-visible">
                ...
                {/* Labels */}
                <text x={startX} y={horizonY + 20} fontSize="10" fill="white" textAnchor="middle">{formatMins(sunriseMins)}</text>
                <text x={endX} y={horizonY + 20} fontSize="10" fill="white" textAnchor="middle">{formatMins(sunsetMins)}</text>
             </svg>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4 w-full mb-8">
            <div className="bg-black/20 rounded-2xl p-3 flex flex-col items-center">
               ...
               <div className="text-lg font-semibold">{formatMins(sunriseMins)}</div>
            </div>
            <div className="bg-black/20 rounded-2xl p-3 flex flex-col items-center">
               <div className="flex items-center gap-1 text-xs text-white/60 mb-1">
                 <Sunset size={12} /> 日落
               </div>
               <div className="text-lg font-semibold">{formatMins(sunsetMins)}</div>
            </div>
          </div>
